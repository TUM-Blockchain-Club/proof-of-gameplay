...
describe("Test Viem SDK AI Agent Contract", () => {
  ...
  // New test case for the proof functionality
  test("Generate and Store Proof of Human Interaction", async () => {
    const resp = await app.request(
      "/?type=proof",
      {},
      {
    ...
    expect(data).toHaveProperty("data");
    expect(data).toHaveProperty("signature");
    expect(data.data).toContain("timestamp"); // Checking proof data includes timestamp
    expect(data.data).toContain("dtwScore"); // Checking proof data includes DTW score
    expect(data.data).toContain("isHuman"); // Checking proof data indicates if human or not
  });
});
